# Authors

This file documents the authors and contributors to the whitepapers in this repository, providing verifiable identity links for attribution and authenticity purposes.

---

## Manuel Antonio Lara Pupo

**Role:** Primary Author, Founder

**Organization:** Unlimitech Cloud LLC

### Bio

Manuel Antonio Lara Pupo is an AI-Driven Solutions Architect with over 20 years of experience building software at scale and 11+ years designing enterprise architecture. He specializes in translating complex business goals into robust, secure solutions—including country-scale platforms. His recent work focuses on production-grade AI enablement: agentic workflows, MCP implementations, and intelligent data pipelines. Manuel is the founder of Unlimitech Cloud LLC, where he combines deep technical expertise with a commitment to sharing knowledge that strengthens the broader engineering community.

### Verifiable Identity

| Platform | Link |
|----------|------|
| ORCID | [0009-0003-6539-950X](https://orcid.org/0009-0003-6539-950X) |
| LinkedIn | [linkedin.com/in/manuel-lara-ai](https://www.linkedin.com/in/manuel-lara-ai/) |
| GitHub | [github.com/manularaai](https://github.com/manularaai) |

### Signing Certificate

All whitepapers are digitally signed using a certificate issued to this author. The public certificate is available at:

- `.publish/certificates/Manuel-Antonio-Lara-Pupo-PUBLIC-2026-01-30.crt`

### Published Works

| Whitepaper | First Published | Current Version |
|------------|-----------------|-----------------|
| [Capability Distillation Driven Development (CDDD / C3D)](Capability%20Distillation%20Driven%20Development/) | January 2026 | v1.1.0 |

---

## Contributors

Contributors who have provided feedback, corrections, or improvements will be acknowledged here.

*No external contributors yet.*

---

## Verification

To verify authorship:

1. Check the digital signature on any signed PDF against the certificate in `.publish/certificates/`
2. Verify the author's identity through the linked profiles above
3. Review the git commit history for full traceability

---

**Last updated:** February 4, 2026
